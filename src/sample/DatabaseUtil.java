...
public class DatabaseUtil {
    ...
    */
    ...
    public void decideApplicationAction(String status, ApplicationData thisForm, javafx.scene.control.TextArea commentsField) throws  SQLException{
        ...
        /*
        String sql = "UPDATE FORM SET GOVID = null WHERE TTBID = ?";
        pstmt = conn.prepareStatement(sql);
        pstmt.setString(1, thisForm.getTtbID());
        pstmt.executeUpdate();
        */
...
/*
    ...
    */
    ...
    public int searchMinWorkLoad() throws SQLException{//TODO: find out fields + name for govt. worker\
        ...
        String minWork = "SELECT AID ,MIN(AMOUNT_OF_WORK) FROM APP.ACCOUNT WHERE USER_TYPE = 1 GROUP BY AID";
        Statement st = conn.createStatement();

        rset = st.executeQuery(minWork);
        if (rset.next()) {
            int id = rset.getInt(1);
            for(Account account: searchAccountWithUserType(1)) {
                if (getAccountAid(account.getUsername()) == id) {
...
*/
    ...
    public void assignForm(int govid, ApplicationData unAssignedForm) throws SQLException{
        ...
        // Update the amount of work
        String getWork = "SELECT * FROM ACCOUNT WHERE AID = ?";
        String updateWork = "UPDATE ACCOUNT SET ACCOUNT.AMOUNT_OF_WORK = ? WHERE AID = ?";
        PreparedStatement ps = conn.prepareStatement(getWork);
        PreparedStatement ps2 = conn.prepareStatement(updateWork);
        ps.setInt(1, govid);
        rset = ps.executeQuery();


        while(rset.next()){
            occ = rset.getInt("AMOUNT_OF_WORK");
        }
